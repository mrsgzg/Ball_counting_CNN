...
class ShuffledBatchWrapper:
    ...
    def __iter__(self):
        for batch in self.dataloader:
            # 获取batch size
            batch_size = batch['sequence_data']['images'].shape[0]
            ...
            shuffled_batch = {
                'sequence_data': {
                    ...
                    'joints': batch['sequence_data']['joints'][perm],  # 🔥 打乱
                    'timestamps': batch['sequence_data']['timestamps'],
...
class ShuffledTemporalWrapper:
    ...
    def __iter__(self):
        for batch in self.dataloader:
            batch_size, seq_len = batch['sequence_data']['images'].shape[:2]
            ...
            for i in range(batch_size):
                ...
                shuffled_joints.append(batch['sequence_data']['joints'][i, perm])
...
if __name__ == "__main__":
    ...
    for i, batch in enumerate(shuffled_temporal_loader):
        if i == 0:
            print(f"  Batch {i}: images shape = {batch['sequence_data']['images'].shape}")
            print(f"  Batch {i}: joints shape = {batch['sequence_data']['joints'].shape}")
            print(f"  Batch {i}: joints[0,:3,0] = {batch['sequence_data']['joints'][0,:3,0]}")

...
class UniversalModelVisualizer:
    ...
    def _generate_joint_pca_report(self, analyzer, save_dir):
        ...
        for label, stats in stats_by_label.items():
            n = stats['total_count']
            stats['avg_circularity'] /= n
            stats['avg_rotations'] /= n
            stats['circular_ratio'] = stats['circular_count'] / n
        
        # 生成报告
        report = {
            'analysis_summary': {
                'total_samples': len(patterns),
                'circular_samples': sum(1 for p in patterns if p['is_circular']),
                'explained_variance': analyzer.analysis_results['joint_pca']['explained_variance_ratio'].tolist(),

...
def run_single_ablation_experiment(
    ...
    try:
        ...
        # 🔥 Monkey patch: 修复训练器以支持No Forward Model消融
        original_compute_loss = trainer.compute_loss
        original_compute_metrics = trainer.compute_metrics
        
        def patched_compute_loss(outputs, targets):
            """修复版compute_loss - 兼容No Forward Model"""
            losses = {}
            device = trainer.device
            
            # 1. 计数损失
            count_logits = outputs['counts']
            batch_size, seq_len, num_classes = count_logits.shape
            count_logits_flat = count_logits.reshape(-1, num_classes)
            labels_flat = targets['labels'].reshape(-1).long()
            valid_mask = labels_flat >= 0
            
            if valid_mask.sum() > 0:
                count_loss = F.cross_entropy(count_logits_flat[valid_mask], labels_flat[valid_mask])
            else:
                count_loss = torch.tensor(0.0, device=device)
            losses['count_loss'] = count_loss
            
            # 2. 🔥 运动损失 - 只在有joints输出时计算
            if 'joints' in outputs and outputs['joints'] is not None:
            ...
            if 'attention_weights' in outputs and outputs['attention_weights'] is not None:
                ...
                if attention_weights.numel() > 0:
                    ...
                    losses['attention_loss'] = torch.tensor(0.0, device=device)
            else:
                losses['attention_loss'] = torch.tensor(0.0, device=device)
            
            # 4. 总损失
            total_loss = (count_loss + 
                         trainer.embodiment_loss_weight * motion_loss +
                         trainer.attention_loss_weight * losses['attention_loss'])
            losses['total_loss'] = total_loss
            
            return losses
        
        def patched_compute_metrics(outputs, targets):
            """修复版compute_metrics - 兼容No Forward Model"""
            metrics = {}
            ...
            if 'joints' in outputs and outputs['joints'] is not None:
                ...
                # 🔥 No Forward Model消融 - 设为0.0表示不适用（避免原始训练器KeyError）
                metrics['joint_mse'] = 0.0
                metrics['joint_mae'] = 0.0
            
            # 每个数字准确率
            for digit in range(11):
                digit_mask = (target_counts == digit) & valid_mask
                if digit_mask.sum() > 0:
                    digit_acc = (pred_labels[digit_mask] == target_counts[digit_mask]).float().mean().item()
                    metrics[f'digit_{digit}_accuracy'] = digit_acc
                else:
                    metrics[f'digit_{digit}_accuracy'] = 0.0
            
            return metrics
        
        # 应用patch
        trainer.compute_loss = patched_compute_loss
        trainer.compute_metrics = patched_compute_metrics
        
        # 🔥 新增：patch validate函数以处理None值
        original_validate = trainer.validate
        
        @torch.no_grad()
        def patched_validate(epoch):
            """修复版validate - 跳过None值的累加"""
            trainer.model.eval()
            total_loss = 0.0
            total_metrics = {}
            ...
            for batch_idx, batch in enumerate(trainer.val_loader):
                # 获取数据（支持扁平和嵌套两种格式）
                if 'images' in batch:
                    # 扁平格式 (Wrapper输出或某些DataLoader)
                    sequence_data = batch['images'].to(trainer.device)
                    labels = batch['labels'].to(trainer.device)
                    joints = batch['joints'].to(trainer.device)
                elif 'sequence_data' in batch:
                    # 嵌套格式 (某些原始DataLoader)
                    sequence_data = batch['sequence_data']['images'].to(trainer.device)
                    labels = batch['sequence_data']['labels'].to(trainer.device)
                    joints = batch['sequence_data']['joints'].to(trainer.device)
                else:
                    raise KeyError(f"Batch missing 'images' or 'sequence_data'. Keys: {batch.keys()}")
                ...
                for key, value in metrics.items():
                    if value is not None:  # 👈 关键：跳过None
                        if key not in total_metrics:
                            total_metrics[key] = 0.0
                        total_metrics[key] += value
            
            # 平均
            num_batches = len(trainer.val_loader)
            avg_loss = total_loss / num_batches
            avg_metrics = {k: v / num_batches for k, v in total_metrics.items()}
            
            # 记录到TensorBoard
            if trainer.writer is not None:
                trainer.writer.add_scalar('Validation/Loss', avg_loss, epoch)
                for key, value in avg_metrics.items():
                    trainer.writer.add_scalar(f'Validation/{key}', value, epoch)
            
            return avg_loss, avg_metrics, None

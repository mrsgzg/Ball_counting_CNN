...
class UniversalModelVisualizer:
        ...
        if t < len(lstm_states):
            ...
            if states_so_far.shape[0] > 2:
                ...
                try:
                    ...
                    ax.plot(states_2d[:, 0], states_2d[:, 1], 'b-', alpha=0.5)
                    ...
                    ax.set_xlabel('PC1')
                    ax.set_ylabel('PC2')
                    ax.set_title(f'LSTM State Trajectory (PCA)')
                    ax.legend()
                except ImportError:
                    # 如果没有sklearn，显示前两个维度
                    ax.plot(states_so_far[:, 0], 'b-', label='Dim 0')
                    if states_so_far.shape[1] > 1:
                        ax.plot(states_so_far[:, 1], 'r-', label='Dim 1')
                    ax.set_xlabel('Frame')
                    ax.set_ylabel('State Value')
                    ax.set_title('LSTM State Evolution')
                    ax.legend()
            else:
                ax.plot(states_so_far[:, 0], 'b-', label='Dim 0')
                if states_so_far.shape[1] > 1:
    ...
    def _plot_prediction_sequence(self, ax, predictions, true_labels, t, 
        ...
        # 添加信息文本
        pred_t = predictions[t].item()
        probs_t = F.softmax(predictions.float(), dim=-1)[t]
        confidence = probs_t[pred_t].item()
...
def main():
    ...
    try:
        ...
        # 生成索引页面
        generate_index_html(sample_indices, save_dir, visualizer.model_info)
...
def generate_index_html(sample_indices, save_dir, model_info):
    ...
    <html>
    <head>
        <title>Universal Model Visualization - {model_info['model_type']}</title>
        <style>
            body {{
    ...
    <body>
        <h1>Universal Model Visualization</h1>
        
        <div class="model-info">
            <h2>模型信息</h2>
            <p><strong>模型类型:</strong> {model_info['model_type']}</p>
            <p><strong>支持功能:</strong> {', '.join(model_info['capabilities'])}</p>
            <div class="capabilities">
                <span>Attention: {'✅' if model_info['has_attention'] else '❌'}</span> |
                <span>Embodiment: {'✅' if model_info['has_embodiment'] else '❌'}</span> |
                <span>Motion Decoder: {'✅' if model_info['has_motion_decoder'] else '❌'}</span>
            </div>
        </div>
    ...
    for idx in sample_indices:
        ...
        html_content += f"""
            <div class="sample-card">
                ...
                <br>
                <a href="{sample_dir}/" target="_blank">查看帧序列</a>
                <a href="{sample_dir}/data.json" target="_blank">查看数据</a>
            </div>
        """
    ...
    html_content += """
        ...
        <div style="margin-top: 40px; text-align: center; color: #666;">
            <p>此可视化工具支持所有模型类型：原始Embodiment模型和Ablation模型</p>
            <p>根据模型能力自动调整可视化内容</p>
        </div>
    </body>
    </html>
